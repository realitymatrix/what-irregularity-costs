...
int main(int argc, char** argv) {
    ...
    for (int arm = 0; arm < 5; ++arm) {
        ...
        else if (arm == 3) a4_allocate_slotidx(&dv, (uint64_t)d, n, 0,0,0,0);
        else osn_tsdf_cuda_allocate_slot_index(v, d, n, 0,0,0,0);
        ...
        // allocation.
        const int32_t nv = -1;
        const char* nm = arm==0?"A3 cuda":arm==1?"A4 64-bit CAS":arm==2?"A4 128-bit CAS":arm==3?"A4 slot-index":"A3 slot-index";

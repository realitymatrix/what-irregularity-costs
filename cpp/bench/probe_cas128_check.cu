...
int main(int argc, char** argv) {
    ...
    printf("=== correctness, %d points ===\n%-14s %10s %10s %14s\n", n, "arm", "blocks", "drops", "vertices");
    printf("--------------------------------------------------------\n");
    ...
    for (int arm = 0; arm < 3; ++arm) {
        ...
        else a4_allocate_cas128(&dv, (uint64_t)d, n, 0,0,0,0);
        osn_tsdf_cuda_synchronize(v);
        osn_tsdf_cuda_update_voxels(v, d, n, 0,0,0,0);
        osn_tsdf_cuda_synchronize(v);
        const int32_t nv = osn_tsdf_cuda_extract_mesh(v, mesh.data(), nullptr, 4<<20, 0.5f, 0.0f);
        printf("%-14s %10d %10llu %14d\n", arm==0?"A3 cuda":(arm==1?"A4 64-bit CAS":"A4 128-bit CAS"),
               osn_tsdf_cuda_block_count(v), (unsigned long long)osn_tsdf_cuda_drop_count(v), nv);
